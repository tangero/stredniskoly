...
### Opraveno
- **Popis školy v souběžných přihláškách jako na stránce oboru** — `scripts/nazvy_oboru.py` bere ročníky katalogu od zobrazeného (registr) ke starším a uvnitř ročníku první záznam v pořadí souboru, stejně jako `nazvyOboru()` na webu; dosud vyhrával starší ročník. Popis se změnil u 36 klíčů (32 škol), doklad `docs/podklady/dopad-precedence-nazvu-2026-09-18.md`. Nejednoznačné klíče (45) generátor hlásí. Ruční pomůcka `scripts/stahni-rejstrik.py` stáhne snímek podle registru a ověří otisk; datová linka ho nepotřebuje, názvy čte z indexu (PR #109)
...
- **Dvě revize téhož období převzaté v jeden den** vytvořily totožný záznam `zobrazeno` v registru — `prepni` u lokálního souboru zapisuje otisk sha256, generátor indexu ho ověří proti snímku a `nacti_index` shodu včetně otisku vyžaduje
...
- **Názvy oborů mimo přehled i v datové lince v CI** — generátory souběžných přihlášek a kontextu přihlášek četly názvy ze snímku rejstříku MŠMT, který v gitu není; přegenerování v CI by názvy smazalo a stránky by se vrátily ke kódům. Názvy se čtou z malého indexu v gitu (`data/msmt_rejstrik/nazvy-oboru.json`, `scripts/build-nazvy-oboru-rejstrik.py`) ze snímku podle registru, chybějící nebo zastaralý index je chyba; ročníky katalogu se berou z registru místo napevno. Značka „mimo přehled“ se ukazuje i u oboru, který má obtížnost přijetí (review Codex)
- **Obory výš a níž na přihlášce ukazovaly obory mimo přehled jen kódem** (např. `600006387_65-51-H/01` se značkou „bez údajů“ na 308 stránkách oborů) — jde hlavně o učební obory bez jednotné zkoušky, které přehled zatím nezahrnuje. Kontext přihlášek k nim nese název školy, obce a oboru z rejstříku škol MŠMT (sdílený `scripts/nazvy_oboru.py`, dříve jen v souběžných přihláškách) a stránka ukazuje značku „bez jednotné zkoušky“ nebo „mimo přehled“ s vysvětlením (slovník pojmů 1.6)
