...
export async function getSchools2026Data(): Promise<School2026Data[]> {
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const staticByNorm = new Map<string, any>();
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const staticByBase = new Map<string, any>();
  const normalizeId = (id: string) => id.normalize('NFD').replace(/[\u0300-\u036f]/g, '').replace(/,/g, '');
  for (const s of schools2025) {
    staticIndex.set(s.id, s);
    // Index podle normalizovaného ID (bez diakritiky) pro matching s 2026 daty
    staticByNorm.set(normalizeId(s.id), s);
  ...
  schools2026Cache = rawRecords.map(r => {
    ...
    const s = staticIndex.get(r.id)
      || staticByNorm.get(normalizeId(r.id))
    ...
    return {
      ...
      nazev: s?.nazev || analysis?.nazev || '',
      nazev_display: s?.nazev_display || s?.nazev || analysis?.nazev || '',
      obor: s?.obor || analysis?.obor || '',
      zamereni: s?.zamereni || analysis?.prev_zamereni_name || '',
      kkov: s?.kkov || '',
      typ: s?.typ || analysis?.typ || '',
      delka_studia: s?.delka_studia || analysis?.delka_studia || 4,
      obec: s?.obec || analysis?.obec || '',
      kraj: s?.kraj || analysis?.kraj || '',
      kraj_kod: s?.kraj_kod || analysis?.kraj_kod || '',
      kapacita: r.kapacita,
