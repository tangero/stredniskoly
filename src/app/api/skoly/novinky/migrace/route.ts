...
import { MIGRACE_SKOLNICH_NOVINEK, TABULKY_SKOLNICH_NOVINEK } from '@/lib/skolni-novinky-schema';

// ============================================================================
// Spuštění migrace školních novinek (RSS) z nasazené aplikace
// (vzor /api/portal/migrace, docs/skolske-novinky-rss-2027.md oddíl 3.3).
//
...
async function stavTabulek(): Promise<{
  ...
  return {
    existuji,
    chybi: TABULKY_SKOLNICH_NOVINEK.filter((t) => !existuji.includes(t)),
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    console.log(`🗄️ Migrace školních novinek: tabulek před ${pred.existuji.length}, po ${po.existuji.length}`);
    return NextResponse.json({ prikazu: MIGRACE_SKOLNICH_NOVINEK.length, pred, po });
  } catch (chyba) {
    console.error('❌ Migrace školních novinek selhala:', chyba);
