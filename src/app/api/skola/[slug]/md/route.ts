...
import { getSchoolPageType, getSchoolOverview, getExtendedSchoolStats, getProgramsByRedizo, getCSIDataByRedizo, getExtractionsByRedizo } from '@/lib/data';
...
export async function GET(
  ...
  const redizo = pageInfo.school.id.split('_')[0];
  ...
  // Load extended stats for each program
  ...
  for (const program of sortedPrograms) {
    ...
    if (stats) {
      ...
      md += `- **Min ČJ:** ${stats.cj_min} bodů z 50\n`;
      md += `- **Min MA:** ${stats.ma_min} bodů z 50\n`;
