...
**Plán ale v `railway.json` není a nemůže být.** Railway bere cron výhradně z nastavení služby; `deploy.cronSchedule` v konfiguračním souboru se **tiše ignoruje**, zatímco `build.dockerfilePath` z téhož souboru zabere. Poznalo se to na tom, že po prvním nasazení bylo `nextCronRunAt` `null` – služba by proběhla jednou při nasazení a víckrát nikdy. Totéž platí pro `deploy.restartPolicyType`. Proto v `railway.json` zůstává jen `build` a plán se nastavuje takto:
