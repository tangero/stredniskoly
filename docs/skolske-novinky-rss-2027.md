...
**Verze:** 1.7 (sklizeň přesunuta na Railway; provozní opravy z prvních běhů; text návrhu po páté oponentuře – [oponentura 1.4](oponentura-skolske-novinky-rss-2027-v1.4.md); starší: [čtvrtá](oponentura-skolske-novinky-rss-2027-v1.3.md), [třetí](oponentura-skolske-novinky-rss-2027-v1.2.md), [druhá](oponentura-skolske-novinky-rss-2027-v1.1.md), [první](oponentura-skolske-novinky-rss-2027.md))
**Datum:** 20. 9. 2026
...
| --- | --- |
| DOD s termíny | karta zvýrazňuje jen budoucí termíny; proběhlý termín se z karty skryje (označí jako proběhlý), nečeká se na poslední. `konec_platnosti` = poslední termín + 3 dny; po něm článek může zůstat novinkou v seznamu, ale **ne jako budoucí pozvánka**. |
| DOD bez termínu | bezpečný fallback: jen titulek+odkaz („škola zveřejnila DOD →"), `konec_platnosti` = publikování + 60 dní. Žádné odvozené datum. |
...
**Jediné publikační rozhodnutí (F5 čtvrté oponentury):** téma, jistota, stav sdělení a termíny ústí do jedné funkce (`rozhodni_publikaci` v měřicím skriptu, v provozu totéž pravidlo), která vrací `karta_terminu` | `karta` | `odkaz` | `seznam` a způsobilost pro e-mail. Testuje se a měří se **ona**, ne mezikroky. Termínová karta vyžaduje **pozitivní vazbu událost–termín–konání**: vysokou jistotu tématu, stav `oznameno`, datum v roli akce, v klauzuli, kterou text neruší, a datum ne starší než článek. Cokoli z toho chybí → původní titulek a odkaz. **Zrušení jednoho z několika termínů neruší ostatní** – stav se drží u klauzule, ne u celého článku.
...
- **Výběr pěti položek dává přednost kartám s termínem (oprava 20. 9. 2026):** blok drží pět položek, ale nevybírají se prostě podle data. Čtecí vrstva bere širší okno posledních 30 platných položek, teprve v nich vyhodnotí platnost termínu a přepínače a do pětice pak řadí nejdřív karty s platným termínem a zbytek podle data. Důvod je naměřený: u školy 600011801 ležela v dávce pozvánka na den otevřených dveří 1. 10. 2026, ale na stránku se nedostala, protože ji přebilo pět novějších zpráv ze života školy – mimo jiné nabídka práce pro dojiče. Pozvánka na den otevřených dveří je přitom jediný údaj, po kterém je v tomhle bloku poptávka. Uvnitř obou skupin se pořadí podle data nemění.
...
**Kde sklizeň běží od 20. 9. 2026: Railway, projekt `stredniskoly-sklizec`.** Obraz staví `Dockerfile.sklizec` (Node 22 + Python 3 s `requests`), běh řídí `scripts/sklizec-beh.sh` a plán drží `railway.json` (`cronSchedule` 04:10 a 14:10 UTC, `restartPolicyType: NEVER`, aby se doběhlá úloha neopakovala). GitHub workflow si **plán zatím nechává** a vypne se až po prvním doloženém úspěšném běhu na Railway se zápisem do `sklizen_beh`. Službě na Railway zatím chybí `DATABASE_URL`, takže by tamní běh skončil hned návratovým kódem 1; kdyby se plán v Actions vypnul dřív, nesklízelo by ani jedno místo a poznalo by se to až z chybějících řádků v `sklizen_beh`. Dokud Railway připojení nemá, jsou obě místa neškodná. Oba běhy zapisují do `sklizen_beh` stejně jako předtím, takže dvojí běh se pozná.
