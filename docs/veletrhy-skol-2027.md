...
**Čipy krajů s počty nahoře** („Jihočeský 6“, „Praha 1“) místo rozbalovacích seznamů: pokrytí je vidět bez kliknutí. Čip je zároveň filtr a výběr se propisuje do adresy (`#jihocesky`), takže ho reload i sdílený odkaz zachovají; kotva při načtení kraj předvybere a po překreslení posune na oddíl. Změna adresy za běhu se sleduje přes `hashchange` i `navigation.currententrychange` (Next při odkazu na tutéž stránku jen s jinou kotvou volá `pushState` bez `hashchange`); vlastní zápis čipem se pozná a stránkou nehýbe — klik na čip nesmí odsunout čipy z obrazovky. Kotva na kraj, kterému akce už proběhly, ukáže „teď o žádné akci nevíme“ s odkazem na stránku kraje — proto seznam dostává všech čtrnáct krajů, ne jen ty s akcí. Odkaz sem ze stránky kraje zatím nikde nevede; kotva je připravená, odkaz je samostatná změna stránky kraje. Filtr měst neexistuje.
