...
2. **Čistá neúspěšnost se neimportuje.** §5.1, metrika 1 ji jmenuje vedle úspěšnosti, ale kontrakt v §4 pro ni pole nemá a `SLOUPCE` v `build-maturita-skoly.py` ji nemapují, přestože ve zdroji je ve všech třech blocích. Doloženo ověřením sloupců, položka `nemapovane_sloupce_v_nasich_blocich`. Na stránce dnes chybí; hlavní číslo je úspěšnost z přihlášených, která neúspěch i neúčast zahrnuje.
