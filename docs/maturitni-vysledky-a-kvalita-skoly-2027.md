...
|---|---|---|---|---|---|
| 1 | Úspěšnost společné části | úspěšní z **přihlášených**, jak ji počítá CERMAT | Kolik prvomaturantů prošlo společnou částí. Pro rodiče nejsrozumitelnější číslo, u SOŠ nejvýznamnější. | Nic o odpadu před maturitou ani o profilové části. | Podíl s počtem a jmenovatelem vedle. |
...
**Čistá neúspěšnost z metriky 1 vypadla 18. 9. 2026.** Do té doby tu stálo „úspěšnost **a čistá neúspěšnost**“ a pokyn „neúspěšnost rozložit na češtinu a matematiku“. Obojí je zamítnuté a důvody jsou v oddílu 10; podstatné je, že rozpad na dva předměty nesedí, protože neúspěch v cizím jazyce nevidíme, a že celkový podíl je určený úspěšností a neúčastí, které na stránce jsou. Jmenovatel metriky 1 je navíc upřesněný: úspěšnost počítá CERMAT z **přihlášených**, ne z konajících (viz slovník ukazatelů, oprava ze 17. 9. 2026).
...
**Ověření mapy sloupců proběhlo 18. 9. 2026** a kroky 1 až 4 níže jsou tím splněné, ovšem pro `build-maturita-skoly.py`, který soubory MZ skutečně zpracovává. Dělá je `scripts/overeni-sloupcu-maturity.py` a opakuje se po každém novém ročníku; doklad je [ověření sloupců](podklady/overeni-sloupcu-maturity.json). Skript porovná názvy sloupců s mapou, vypíše sloupce, které v našich blocích jsou a mapa je nebere, dopočítá podíly z počtů proti definicím z vysvětlivek a ohlídá, že medián směrodatné odchylky nepřeroste medián percentilu. Soubory JPZ 2017–2023, kde past z §9.1 skutečně je, zůstávají neověřené, protože se nestahují.
...
**Vypořádáno 18. 9. 2026:**

Čísla odpovídají pětici položek, kterou vypsala verze 1.2, aby se daly dohledat starší odkazy; položka 1 zůstává otevřená a je níž. Šestá vznikla až při ověření sloupců.

2. **Neúčast jako samostatný podíl** (§5.1, metrika 5). **Hotovo.** V tabulce po letech na stránce školy je sloupec „ke zkoušce nešlo“ s počtem a podílem z přihlášených (`absent`, `nonParticipationRate`), bez výkladu důvodu. Umístění v důkazní tabulce, ne v hlavní kartě, plyne ze [soupisu zdrojů](zdroje-dat.md): hlavní číslo je úspěšnost s jmenovatelem, neúčast je důkaz. Počet nekonajících u poslední maturity se v kartě oboru zobrazoval už od revize 14. 9. 2026; chyběl jen podíl a řada let.
3. **Ověření mapy sloupců proti listu `vysvetlivky`** (§9.1). **Hotovo, bez nálezu.** `scripts/overeni-sloupcu-maturity.py`, doklad [ověření sloupců](podklady/overeni-sloupcu-maturity.json). Ověřeno na jarních souborech 2021 až 2026 stažených 18. 9. 2026; otisky 2023 až 2026 se shodují s těmi, které nese `public/maturita_skoly.json`, a otisk 2026 s auditem R1. Shoda názvu sloupce ale nestačí, proto skript **dopočítává podíly z počtů**: `PODÍL ÚSPĚŠNÝCH`, `ČISTÁ` a `HRUBÁ NEÚSPĚŠNOST` a `NEÚČAST` sedí na definice z vysvětlivek do posledního místa ve všech 3 375 řádcích každého ročníku. Past z §9.1 v souborech MZ nehrozí: `SMĚRODATNÁ ODCHYLKA % SKÓRU` a `PRŮMĚRNÉ PERCENTILOVÉ UMÍSTĚNÍ` jsou samostatné sloupce a jejich mediány se liší o řád (12,2 proti 44,3). Pozor na záměnu: nad skutečnými soubory běží `build-maturita-skoly.py`, nikoli `maturita_prototype.py`; ten zůstává na syntetických datech, protože slouží výzkumné vrstvě 3.
4. **Testy na organizační změny** (přejímací podmínka 7). **Hotovo.** `tests/test_maturita_skoly.py`, třída `TestOrganizacniZmeny`: škola se třemi `SMO16` (školní agregát se nerozkopíruje, do reference každé skupiny vstoupí jednou), změna REDIZO (řada se nespojí), sloučení škol (nástupce nepřevezme historii), zaniklá skupina oborů, a duplicitní řádek téže školy a skupiny. Při té příležitosti se zjistilo, že skript **neměl kontrolu duplicit**, kterou žádá přejímací podmínka 3: druhý řádek téhož klíče by první tiše přepsal. Kontrola doplněna, ve skutečných souborech 2021 až 2026 je duplicit nula.
5. **Ročníky 2015–2022.** **Zamítnuto, doloženo měřením.** Položka byla navíc špatně zapsaná: roky 2015 až 2020 zamítl už §2.1 kvůli zlomu metodiky a `build-maturita-skoly.py` je odmítá (`PRVNI_ROK = 2021`), takže ve hře byly jen ročníky 2021 a 2022. Ty jsou dostupné a neškodné — schéma souborů je ve všech šesti ročnících totožné (98 sloupců, 8 bloků) a jejich přidání nezměnilo ani jednu ze 40 074 stávajících hodnot. Důvod, proč se přesto nepřebírají, je ten, že **nesplní to, kvůli čemu byly na seznamu**. Delší řada měla zpevnit „počet let nad skupinou“, jenže předpovědní schopnost okna je plochá: většinové zařazení z jednoho roku trefí ročník 2026 v 62,1 %, ze dvou v 65,6 %, ze tří v 65,7 %, ze čtyř v 64,8 % a z pěti v 66,2 %. Stejné plato vychází i u malých škol pod 30 konajícími (65,1 / 64,6 / 64,9 / 65,4 %), tedy právě tam, kde měl jeden rok být šum. Šestileté okno by přitom znění změnilo u 403 z 1 627 škol ve skupině oborů a o 156 škol zmenšilo pokrytí, protože starší zařazení nemají. Doklad: `scripts/delka-rady-maturity.py`, [délka řady](podklady/delka-rady-maturity.json). **Rozhodnutí se mění jen novým argumentem, ne novým ročníkem**; přibude-li jaro 2027, okno zůstává čtyřleté a posune se.

6. **Čistá neúspěšnost** (§5.1, metrika 1). **Zamítnuta, metrika 1 opravena.** Objevila se při ověření sloupců jako jediná položka `nemapovane_sloupce_v_nasich_blocich`: ve zdroji je ve všech třech blocích, kontrakt §4 pro ni pole nemá. Čtyři důvody, proč to tak zůstane:

   - **Není to mezera v datech.** Čistá neúspěšnost je `NEUSPĚLI / KONALI` a obě pole importujeme jako `failed` a `took`. Identita `přihlášení = úspěšní + neúspěšní + nekonající` platí ve všech 3 375 záznamech roku 2026 a ověření sloupců ukázalo, že `failed / took` reprodukuje sloupec CERMATu do posledního místa. Jde tedy o rozhodnutí o zobrazení, ne o import.
   - **Celkový podíl nepřidá nic nového.** Stránka ukazuje úspěšné z přihlášených a nekonající z přihlášených; třetí podíl je těmi dvěma plně určený. Čtvrté číslo do bloku jde proti revizi 8.2 [stránky školy](stranka-skoly-2027.md), která z něj čísla naopak odebírala ve prospěch jedné odpovědi.
   - **Rozpad na češtinu a matematiku by nesouhlasil.** Ze 2 266 záznamů škola a skupina oborů za rok 2026 nemá 593 jediného neúspěšného. Ze zbylých 1 673 jich 1 035 neuspělo jen v češtině, 299 v obou předmětech, 175 jen v matematice a **164 ani v jednom**: propadli v cizím jazyce, jehož bloky jsou zavržené (malé skupiny, samovýběr). Rozpad by u těch 164 ukázal nulu a nulu, přestože neúspěšní jsou. Medián neúspěšných je u škol, které je mají, tři lidé.
   - **Účel metriky plní neúčast.** Důvod, proč §5.1 chtěl vedle úspěšnosti i čistou neúspěšnost, je survivorship z §2.1: škola, která slabé žáky k maturitě nepustí, bude mít vysokou úspěšnost. Rozdíl obou jmenovatelů jsou přesně ti, kdo ke zkoušce nešli, a ti jsou od 18. 9. 2026 v tabulce po letech jako vlastní sloupec.

   **Kdyby se někdy zobrazovat měla, odvodí se z `failed / took`, neimportuje se.** Odvozená hodnota respektuje meze zveřejnění z §7 (pod deseti konajícími se podíly potlačují), kdežto převzatý sloupec CERMATu by je obcházel. Ze stejného důvodu se zamítá i varianta „doimportovat do kontraktu, ale nezobrazovat“ kvůli exportu otevřených dat na `/skola/<slug>.json`: přidala by pole, které slovník nevede a stránka nepoužívá, a v exportu by se objevilo i tam, kde je potlačené.

**Není hotové (stav k 18. 9. 2026):**

1. **Směrodatná odchylka slovně** (§5.1, metrika 4). Pole `standardDeviation` se počítá jen jako vstup do standardní chyby; věta „výsledky vyrovnané / rozptýlené“ na stránce není. Ponecháno vědomě: revize 8.2 [stránky školy](stranka-skoly-2027.md) šla opačným směrem, tedy k jedné odpovědi místo výčtu čísel, a další nálepka vedle „nad středem podobných škol“ by obraz rozostřila. Data v souboru jsou, takže se to dá kdykoli doplnit.

Je to jediná otevřená položka oddílu 10.
