...
for (const route of [`/skola/${slug}`, `/skola/${slug}/detail`, `/skola/${slug}/pro-me`, '/skola/600007774-gymnazium-j-s-machara-kralovicka', '/skoly', '/vysledky/2026', '/regiony/stredocesky']) {
  test(`P0 veřejný text a metadata: ${route}`, async () => {
    ...
    assert.doesNotMatch(text, /Min\. skóre pro přijetí \(2025\):|Šance přijetí podle priority|Splňujete bodové požadavky|rodiče reagují|Nejobtížnější obory|Náročnost přijímaček/);
...
test('JSON a Markdown nemají numerickou hranici přijetí', async () => {
  ...
  assert.doesNotMatch(await md.text(), /Minimální body pro přijetí:\*\* \d/);
});
